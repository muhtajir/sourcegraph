...
func (r *Resolver) readRecipientQuery(ctx context.Context, emailId int64) (*sqlf.Query, error) {
	const readRecipientQuery = `
SELECT email, namespace_user_id, namespace_org_id
FROM cm_recipients
WHERE email = %s;
`
	return sqlf.Sprintf(
...
func (m *monitorEmail) Recipients(ctx context.Context, args *graphqlbackend.ListRecipientsArgs) (c graphqlbackend.MonitorActionEmailRecipientsConnectionResolver, err error) {
	q, err := m.readRecipientQuery(ctx, m.id)
	if err != nil {
		return nil, err
	}
	...
	return &monitorActionEmailRecipientsConnection{ns}, nil
}
